...
num_epochs = 30
num_epochs_cifar = 3*num_epochs
checks = 300
batch_size = 128
learning_rate = 1e-3
for point in range(N_0, N_0+N_points):
	if point >= N_0 + 1:
		del model, optimizer, criterion, train_loader, test_loader, data, loss, loss_h, loss_v
	# num_epochs = 20
	# num_epochs_cifar = 2*num_epochs
	

	folder = '/raid/data/matlab/sad/ResNet/'
	datasets = ['../mnist_corrupted.npz', '../fashion_mnist_corrupted.npz', '../cifar10_corrupted.npz']

	for dataset in datasets:
		print('%d ResNet on %s, usual'%(point, dataset[3:]))
	...
	for dataset in datasets:
		print('%d ResNet on %s, sad'%(point, dataset[3:]))
		if dataset != '../cifar10_corrupted.npz':
			model_def = reload(model_def)
			model = model_def.model
			model.apply(weight_init)
