...
# #         mnist.train.labels = y_train_c
        ...
        test_loss = cross_entropy.eval(feed_dict={x: mnist.test.images, y_: mnist.test.labels})
        train_accuracy_usual = accuracy.eval(feed_dict={x: mnist.train.images, y_: mnist.train.labels})
        train_loss_usual = cross_entropy.eval(feed_dict={x: mnist.train.images, y_: mnist.train.labels})
        ...
        pylab.title('BAD_Adam, MLP%d batch %d'%(h,bat), fontsize=16)
        pylab.ylabel("Accuracy", fontsize=16)
        pylab.xlabel("Steps x100", fontsize=16)
        plt.legend()
        plt.savefig('BAD_Acc_Adam, MLP%d batch %d.pdf'%(h,bat))
        plt.clf()
        ...
        pylab.xlabel("Steps x100", fontsize=16)
        plt.legend()
        plt.savefig('BAD_Loss_Adam, MLP%d batch %d.pdf'%(h,bat))
        plt.clf()
