...
H = [512]
batch_s = [64]
regular = [0.01, 0.001, 0.0001, 0.00001, 0]
for h in H:
    for bat in batch_s:
        for reg in regular:
            ...
            W_fc10 = tf.placeholder_with_default(4*tf.random_normal([784, h], stddev=0.1), [784, h])
            b_fc10 = tf.placeholder_with_default(4*tf.random_normal([h], stddev=0.1), [h])
            W_fc20 = tf.placeholder_with_default(4*tf.random_normal([h, 10], stddev=0.1), [h,10])
            b_fc20 = tf.placeholder_with_default(4*tf.random_normal([10], stddev=0.1), [10])
            ...
            cross_entropy = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y)) + reg*tf.abs((tf.norm(W_fc1) + tf.norm(b_fc1) + tf.norm(W_fc2) + tf.norm(b_fc2)) - 300) #regularizer
            ...
            Gradients = []
            Distance = []
            pt = progress_timer(description= 'Finding sad H%d B%d R%f'%(h, bat, reg), n_iter=N_iter)
            ...
            fig.suptitle('MLP H%d B%d R%f'%(h, bat, reg), fontsize=16)
            ...
            plt.plot()
            plt.savefig('Finding sad_MLP H%d B%d R%f.pdf'%(h, bat, reg))
            plt.clf()
            ...
            pt = progress_timer(description= 'Finding nearest local H%d B%d R%f'%(h, bat, reg), n_iter=N_iter)
            for i in range(N_iter):
            ...
            fig = plt.figure(figsize=(12, 15)) 
            fig.suptitle('MLP H%d B%d R%f; d(init, sad) = %f; d(sad, local) = %f'%(h,bat,reg, d_init_sad, d_sad_loc), fontsize=16)
            ...
            plt.plot()
            plt.savefig('Finding local_MLP H%d B%d R%f.pdf'%(h, bat, reg))
            plt.clf()